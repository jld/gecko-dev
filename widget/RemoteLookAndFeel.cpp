...
// static
RemoteLookAndFeel* RemoteLookAndFeel::Get() { return sSingleton; }
...
bool RemoteLookAndFeel::GetFontImpl(FontID aID, nsString& aFontName,
  ...
  size_t index =
      static_cast<size_t>(aID) - static_cast<size_t>(FontID::MINIMUM);
...
FullLookAndFeel RemoteLookAndFeel::ExtractData(nsXPLookAndFeel* aImpl) {
  ...
  for (size_t i = static_cast<size_t>(FontID::MINIMUM);
    ...
    bool rv =
        aImpl->GetFontImpl(static_cast<FontID>(i), font.name(), fontStyle);
