...
ContentParent::SetPriorityAndCheckIsAlive(ProcessPriority aPriority)
{
    ...
    //
    // Hack for bug 943174: use waitid() with WNOWAIT so that, if the
    // process did exit, we won't consume its zombie and confuse the
    // GeckoChildProcessHost dtor; also, if the process isn't a direct
    // child because of Nuwa this will fail with ECHILD, and we need
    // to assume the child is alive in that case rather than assuming
    // it's dead.
#ifdef XP_UNIX
    siginfo_t info;
    info.si_pid = 0;
