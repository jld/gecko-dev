...
// FIX THESE; they need to be async signal safe.
#define RAW_LOG(level, str) CHROMIUM_LOG(level) << (str)
#define RAW_CHECK(cond) CHECK(cond)
// Also not quite right.
#define PCHECK(cond) CHECK(cond)
#define PLOG(level) CHROMIUM_LOG(level)
