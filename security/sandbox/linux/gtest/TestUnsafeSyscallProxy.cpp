...
// The explicit operator bool doesn't work with ASSERT_TRUE.
#define ASSERT_SOME(o) ASSERT_TRUE((o).isSome())

namespace mozilla {

// Name begins with "Sandbox" for more convenient test wildcarding.
...
protected:
  ...
  // Convenience function for sending syscall requests with a mix of
  // pointer/int/long arguments.
  ...
  // openat is a superset of open, and new architectures (arm64) have
  // only openat.
  Maybe<long> Open(const char* aPath, int aFlags, int aMode = 0666) {
    return Syscall(__NR_openat, AT_FDCWD, aPath, aFlags, aMode);
  }

  // Avoid code duplication for a common task.
  ...
  // Utilities for running a method of this class (or a subclass) on a
  // separate thread.
...
// Opening a file and using the fd in the client thread.
...
// A few of the syscalls that shouldn't be proxied; not an exhaustive
// duplication of the list in IsProxiable, but just checking that the
...
// Instead, just make sure they're in the same process.
TEST_F(SandboxUnsafeProxyTest, IsInSameProcess)
{
  auto proxyPid = Syscall(__NR_getpid);
  ASSERT_SOME(proxyPid);
  EXPECT_EQ(getpid(), *proxyPid);
  // Alternate approach: proxied faccessat on /proc/self/task/N where
  // N is the test's tid.
}

// Make lots of requests from multiple threads concurrently and try to
// verify that they don't get mixed up.
...
void SandboxUnsafeProxyTest::SimpleOpenTest() {
  auto nullfd = Open("/dev/null", O_RDONLY);
  auto zerofd = Open("/dev/zero", O_RDONLY);
  ASSERT_SOME(nullfd);
  ASSERT_SOME(zerofd);
  ASSERT_GE(*nullfd, 0);
  ASSERT_GE(*zerofd, 0);
...
// And now with async signals.  (This would be simpler with
// pthread_sigqueue, but the B2G ICS emulator is too old to support
// it.)  The goal is to have a thread that's repeatedly calling the
// proxy, and is interrupted at all points in that process with a
// signal handler that also uses the proxy (with enough checks that
// any mix-ups will be caught).
...
TEST_F(SandboxUnsafeProxyTestWithSignals, TheTest)
{
  ...
  // Send them some async signals.
  for (int i = 0; i < kNumLoops; ++i) {
    if (i % (kNumThreads + 1) == 0) {
      // Send a request to wait for the proxy to become available;
      // this doesn't guarantee that the worker threads will get a
      // chance to make progress, but might help.
      int fds[2];
