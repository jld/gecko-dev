...
// See UnsafeSyscallProxy.h for why this exists and an overview of
// what it does.
...
// async signal context (e.g., open).  There's no reliable way to
// check for async signal context, so we can't simply fail in that
// case or assert that it's not used.
//
// In particular, this means that the pthread mutex/condvar facilities
// cannot be used to synchronize communication between the clients and
// the proxy thread.  Those operations are permitted to use global or
// thread-local state (and at least glibc appears to do so), which
// could become inconsistent if *any* synchronization operation, even
// on an unrelated mutex, is interrupted.
//
// Therefore, atomic integers are used for inter-thread communication,
// with the help of futex(2) to allow blocking rather than just
// spin-waiting, and sigprocmask(2) to manage reentrancy (so that this
// code doesn't have to be "NMI-safe").
...
static void AtomicWait(Atomic<int>& aInt, int aVal)
{
  ...
  if (syscall(__NR_futex, uaddr, FUTEX_WAIT, aVal, timeout) != 0) {
    MOZ_ASSERT(errno == EINTR || errno == EWOULDBLOCK);
  }
}
...
{
  int *uaddr = reinterpret_cast<int*>(&aInt);
  MOZ_ALWAYS_TRUE(syscall(__NR_futex, uaddr, FUTEX_WAKE, aNumThreads) >= 0);
}

class UnsafeSyscallProxyImpl MOZ_FINAL {
  enum { kMaxSyscallArgs = UnsafeSyscallProxy::kMaxSyscallArgs };
  pthread_t mThread;
  // mCurrentClient contains the ID (see gettid(2)) of the thread
  // currently communicating with the proxy, or 0 if none.  This acts
  // as a simple mutex which allows detecting reentrancy.
  ...
  unsigned long mSyscall, mArgs[kMaxSyscallArgs];
  long mResult;
  ...
  enum Status {
    ...
    // of starting or exiting).
    STOPPED,
  };

  // A RAII class to temporarily block most signals.
  //
  // This does not block SIGSYS, because if we somehow caused a
  // seccomp trap to occur in that state, then the kernel would both
  // unblock the signal and remove our handler before posting the
  // signal, thus immediately killing the process.  It's better to
  // leave it unblocked and detect the reentrant invocation of the
  // syscall proxy client, so we have a chance to report the error.
  class BlockSignals {
    sigset_t mOldMask;
  public:
    BlockSignals() {
      sigset_t newMask;
      MOZ_ALWAYS_TRUE(sigfillset(&newMask) == 0);
      MOZ_ALWAYS_TRUE(sigdelset(&newMask, SIGSYS) == 0);
      MOZ_ALWAYS_TRUE(sigprocmask(SIG_SETMASK, &newMask, &mOldMask) == 0);
    }
    ~BlockSignals() {
      MOZ_ALWAYS_TRUE(sigprocmask(SIG_SETMASK, &mOldMask, nullptr) == 0);
    }
  };
  ...
  bool CallInternal(unsigned long aSyscall,
                    const unsigned long aArgs[kMaxSyscallArgs],
                    long *aResult,
                    DebugOnly<int> aExpected) {
    if (mStatus == STOPPED) {
      return false;
    }
...
public:
  ...
  bool Start(void) {
    ...
    // Wait for the thread to change mStatus; otherwise Call() will
    // spuriously fail.
    while (true) {
      int status = mStatus;
      if (status != STOPPED) {
  ...
  bool Call(unsigned long aSyscall,
            const unsigned long aArgs[kMaxSyscallArgs],
            long *aResult) {
  ...
  bool Stop() {
    unsigned long args[kMaxSyscallArgs] = { 0, };
...
bool
UnsafeSyscallProxy::Call(unsigned long aSyscall,
                         const unsigned long aArgs[kMaxSyscallArgs],
                         long *aResult)
{
