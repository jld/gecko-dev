...
void SandboxFilterImpl::AllowThreadClone() {
  ...
  static const int flags = flags_common | CLONE_DETACHED;
#elif ANDROID_VERSION < 21
  static const int flags = flags_common;
#else // ANDROID_VERSION >= 21
  static const int flags = flags_common | CLONE_SETTLS |
    CLONE_PARENT_SETTID | CLONE_CHILD_CLEARTID;
#endif // ANDROID_VERSION
#else // not Android; assume glibc
  static const int flags = flags_common | CLONE_SETTLS |
    CLONE_PARENT_SETTID | CLONE_CHILD_CLEARTID;
#endif
  ...
  Allow(SYSCALL_WITH_ARG(clone, 0, flags));
}
