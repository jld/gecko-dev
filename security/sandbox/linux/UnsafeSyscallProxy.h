...
//
// This can't work with syscalls that affect the calling thread or
// otherwise care what thread they're run on, but typically these
// either can safely be allowed by the sandbox policy (and so don't
// need to be proxied) or aren't used by the code being sandboxed.
//
...
public:
  enum {
    kMaxSyscallArgs = 6
    // Note: this will have to change to support MIPS (8 args).
  };
