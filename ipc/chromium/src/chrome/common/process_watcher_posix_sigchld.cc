...
// process has exited.
//
// FIXME yet again, the comment
static bool IsProcessDead(pid_t pid, BlockingWait aBlock) {
  int info;
  switch (WaitForProcess(pid, aBlock, &info)) {
    case base::ProcessStatus::RUNNING:
      // FIXME FIXME FIXME this can happen in the "blocking" case;
      // whose fault is that?
      return false;
  ...
  void WillDestroyCurrentMessageLoop() override {
    ...
    if (children) {
      for (const auto& child : *children) {
        ...
        while (IsProcessDead(child.mPid, BlockingWait::YES) == false) {
          // FIXME copy the comment about this over.  Also, if I use
          // sched_yield, it spins the loop ~200 times per process, lolsob.
          sleep(1);
        }
      }
      children = nullptr;
