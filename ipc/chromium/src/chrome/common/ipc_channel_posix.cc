...
void Channel::ChannelImpl::SetPipe(int fd) {
  ...
  if (fd >= 0) {
    ...
    if (getsockopt(fd, SOL_SOCKET, SO_SNDBUF, &pipe_buf_len_, &optlen) != 0) {
      CHROMIUM_LOG(WARNING) << "Unable to determine pipe buffer size: " << strerror(errno);
...
bool Channel::ChannelImpl::ProcessOutgoingMessages() {
  ...
  while (!output_queue_.IsEmpty()) {
    ...
    // OS-dependent limits.
    while (!iter.Done() && iov_count < kMaxIOVecSize && PipeBufCanHold(amt_to_write)) {
      char* data = iter.Data();
    ...
    if (intentional_short_write || static_cast<size_t>(bytes_written) != amt_to_write) {
      // If write() fails with EAGAIN then bytes_written will be -1.
      if (bytes_written > 0) {
        MOZ_DIAGNOSTIC_ASSERT(intentional_short_write || static_cast<size_t>(bytes_written) <
                              amt_to_write);
