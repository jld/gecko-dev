...
#  define SHM_OPEN_IMPL SafeShmOpen
#  define SHM_UNLINK_IMPL SafeShmUnlink
#else
#  define SHM_OPEN_IMPL(freezeable, name, flag, mode) \
    (shm_open(name, flag, mode))
#  define SHM_UNLINK_IMPL(freezeable, name) (shm_unlink(name))
#endif  // OS_MACOSX
...
bool SharedMemory::Create(size_t size, bool freezeable) {
  read_only_ = false;
  ...
  do {
    ...
    fd.reset(HANDLE_EINTR(SHM_OPEN_IMPL(freezeable, name.c_str(),
                                        O_RDWR | O_CREAT | O_EXCL, 0600)));
    if (fd) {
      if (freezeable) {
        frozen_fd.reset(HANDLE_EINTR(
            SHM_OPEN_IMPL(freezeable, name.c_str(), O_RDONLY, 0400)));
        if (!frozen_fd) {
      ...
      if (SHM_UNLINK_IMPL(freezeable, name.c_str()) != 0) {
