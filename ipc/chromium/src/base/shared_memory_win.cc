...
bool SharedMemory::CreateInternal(size_t size, bool freezeable) {
  ...
  mapped_file_.reset(CreateFileMapping(INVALID_HANDLE_VALUE, psa, PAGE_READWRITE, 0,
...
bool SharedMemory::ReadOnlyCopy(SharedMemory* ro_out) {
  ...
  HANDLE ro_handle;
  if (!::DuplicateHandle(GetCurrentProcess(), mapped_file_.release(), GetCurrentProcess(),
...
bool SharedMemory::Map(size_t bytes, void* fixed_address) {
  ...
  void* mem = MapViewOfFileEx(
      mapped_file_.get(), read_only_ ? FILE_MAP_READ : FILE_MAP_READ | FILE_MAP_WRITE,
